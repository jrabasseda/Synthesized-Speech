...
public class ProsodyElementHandler {
	...
	public void process(Document doc) {
		...
		while ((e = (Element) tw.nextNode()) != null) {
			...
			boolean hasThematicityAttribute = e.hasAttribute("thematicity");
			...
			// if prosody element contains 'thematicity' attribute, apply rate specifications
			if (hasThematicityAttribute) {
				applyThematicityRateSpecifications(nl, e.getAttribute("thematicity"));
			}
			
			// if prosody element contains any 'pitch' modifications
			if (hasPitchAttribute || hasContourAttribute || hasThematicityAttribute) {
				...
				// Set pitch modifications to base contour (first order polynomial fit contour)
				if (hasThematicityAttribute) {
					baseF0Contour = applyThematicityPitchSpecifications(nl, baseF0Contour, e.getAttribute("thematicity"));
				}
	...
	private void applySpeechRateSpecifications(NodeList nl, String rateAttribute) {
		...
		boolean hasLabel = rateAttribute.equals("xx-slow") || rateAttribute.equals("x-slow") || rateAttribute.equals("slow")
				|| rateAttribute.equals("medium") || rateAttribute.equals("fast") || rateAttribute.equals("x-fast") || rateAttribute.equals("default");
		if (hasLabel) {
	...
	/**
	 * To apply 'thematicity rate' specifications
	 * 
	 * 
	 * 
	 */
	private void applyThematicityRateSpecifications(NodeList nl, String thematicityAttribute) {

		if ("".equals(thematicityAttribute)) {
			return;
		}
		
		// if the format contains a fixed label
		boolean hasLabel = thematicityAttribute.equals("theme") || thematicityAttribute.equals("rheme");
		if (hasLabel = thematicityAttribute.equals("theme")) { // theme rate process
			
			thematicityAttribute = rateLabels2RelativeValues(thematicityAttribute);
			
			// Pattern p = Pattern.compile("[+|-]\\d+%");
			Pattern p = Pattern.compile("[+|-][0-9]+(.[0-9]+)?[%]?");
			// Split input with the pattern
			Matcher m = p.matcher(thematicityAttribute);
			if (m.find()) {
				double percentage = new Double(thematicityAttribute.substring(1, thematicityAttribute.length() - 1)).doubleValue();
				if (thematicityAttribute.startsWith("+")) {
					modifySpeechRate(nl, percentage, true);
				} else {
					modifySpeechRate(nl, percentage, false);
				}
			}
		} else if (hasLabel = thematicityAttribute.equals("rheme")) { //rheme rate process
	...
	/**
	 * To apply 'thematicity pitch' specifications
	 * 
	 * 
	 * 
	 */
	private double[] applyThematicityPitchSpecifications(NodeList nl, double[] baseF0Contour, String thematicityAttribute) {
		...
		// if the format contains a fixed label
		boolean hasLabel = thematicityAttribute.equals("theme") || thematicityAttribute.equals("rheme");
		if (hasLabel = thematicityAttribute.equals("theme")) { //theme pitch process
			
			thematicityAttribute = pitchLabels2RelativeValues(thematicityAttribute);

			if (thematicityAttribute.startsWith("+")) {
				if (thematicityAttribute.endsWith("%")) { // type example: +20%
					double modificationPitch = (new Float(thematicityAttribute.substring(1, thematicityAttribute.length() - 1))).doubleValue();
					for (int i = 0; i < baseF0Contour.length; i++) {
						baseF0Contour[i] = baseF0Contour[i] + (baseF0Contour[i] * (modificationPitch / 100.0));
					}
				}
			} else if (thematicityAttribute.startsWith("-")) {
				if (thematicityAttribute.endsWith("%")) { // type example: -20%
					double modificationPitch = (new Float(thematicityAttribute.substring(1, thematicityAttribute.length() - 1))).doubleValue();
					for (int i = 0; i < baseF0Contour.length; i++) {
						baseF0Contour[i] = baseF0Contour[i] - (baseF0Contour[i] * (modificationPitch / 100.0));
		...
		} else if (hasLabel = thematicityAttribute.equals("rheme")) { //theme pitch process
	...
	private double[] applyPitchSpecifications(NodeList nl, double[] baseF0Contour, String pitchAttribute) {
		...
		boolean hasLabel = pitchAttribute.equals("x-low") || pitchAttribute.equals("low") || pitchAttribute.equals("medium")
				|| pitchAttribute.equals("high") || pitchAttribute.equals("x-high") || pitchAttribute.equals("default") || pitchAttribute.equals("xx-high");
	...
	private String rateLabels2RelativeValues(String rateAttribute) {
		...
		if (rateAttribute.equals("xx-slow")) {
			return "-75%";
		} else if (rateAttribute.equals("x-slow")){
			return "-50%";
			...
			return "+100%";
		} else if (rateAttribute.equals("theme")) {
			return "+5%";
		} else if (rateAttribute.equals("rheme")) {
			return "-5%";
		}
	...
	private String pitchLabels2RelativeValues(String pitchAttribute) {
		...
		if (pitchAttribute.equals("x-low")) {
			...
			return "+200%";
		} else if (pitchAttribute.equals("xx-high")) {
			return "+300%";
		} else if (pitchAttribute.equals("theme")) {
			return "+20%";
		} else if (pitchAttribute.equals("rheme")) {
			return "+10%";
		}
